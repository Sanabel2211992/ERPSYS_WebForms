    ...
    public partial class QuoteForm : Page
    {
        ...
        protected void rgQuote_PreRender(object sender, EventArgs e)
        {
            ...
            if (NewMainQuoteLineId > 0)
            {
                foreach (GridDataItem item in rgQuote.MasterTableView.Items)
                {
                    if (item.GetDataKeyValue("ParentId").ToInt() == -1 && item.GetDataKeyValue("ItemId").ToInt() == -1 && item.GetDataKeyValue("QuoteLineId").ToInt() == NewMainQuoteLineId)
                    {
                        item.Expanded = true;
                        NewMainQuoteLineId = -1;
                    }
        ...
        protected void rgQuote_InsertCommand(object sender, GridCommandEventArgs e)
        {
            ...
            try
            {
                if (e.Item.OwnerTableView.Name == "MainItem")
                {
                    ...
                    AddQuoteMainLine(itemId, partNumber, itemCode, description, quantity, unitPrice, profit, discount, isPercentDiscount);
                }
                else if (e.Item.OwnerTableView.Name == "SubItems")
                {
                    GridDataItem dataItem = e.Item.OwnerTableView.ParentItem;
        ...
        protected void rgQuote_UpdateCommand(object sender, GridCommandEventArgs e)
        {
            ...
            try
            {
                if (e.Item.OwnerTableView.Name == "MainItem")
                {
                    ...
                    UpdateQuoteLMainLine(quoteLineId, itemId, description, quantity, unitPrice, profit, discount, isPercentDiscount);
                }
                else if (e.Item.OwnerTableView.Name == "SubItems")
                {
        ...
        protected void AddQuoteMainLine(int itemId, string partNumber, string itemCode, string descriptionAs, decimal quantity, decimal unitPrice, decimal profit, decimal discount, bool isPercentDiscount)
        {
        ...
        protected void UpdateQuoteLMainLine(int quoteLineId, int itemId, string description, decimal quantity, decimal unitPrice, decimal profit, decimal discount, bool isPercentDiscount)
        {
            QuoteLine line = new QuoteLine();
        ...
        protected void AddQuoteSubLine(int parentId, int itemId, string partNumber, string itemCode, string descriptionAs, decimal quantity, decimal unitPrice, decimal profit, decimal discount, bool isPercentDiscount)
        {
            ...
            rgQuote.RetainExpandStateOnRebind = true;
        ...
        protected void btnMoveUp_Click(object sender, System.Web.UI.ImageClickEventArgs e)
        {
            try
            {
                rgQuote.MasterTableView.IsItemInserted = false;
                ...
                if (index - 1 != -1)
                {
                    int movedItemId = rgQuote.MasterTableView.DataKeyValues[index]["QuoteLineId"].ToInt();
                    int nextItemId = rgQuote.MasterTableView.DataKeyValues[index - 1]["QuoteLineId"].ToInt();
                    ...
                    rgQuote.RetainExpandStateOnRebind = false;
                    rgQuote.Rebind();
                }
            }
            catch (Exception ex)
        ...
        protected void btnMoveDown_Click(object sender, System.Web.UI.ImageClickEventArgs e)
        {
            try
            {
                ...
                int index = item.ItemIndex;
                if (index + 1 < rgQuote.MasterTableView.Items.Count)
                {
                    int movedItemId = rgQuote.MasterTableView.DataKeyValues[index]["QuoteLineId"].ToInt();
                    int nextItemId = rgQuote.MasterTableView.DataKeyValues[index + 1]["QuoteLineId"].ToInt();
        ...
        protected void btnMoveUpSub_Click(object sender, System.Web.UI.ImageClickEventArgs e)
        {
            try
            {
                ImageButton upbtn = (ImageButton)sender;
                ...
                if (index - 1 != -1)
                {
                    int movedItemId = item.OwnerTableView.DataKeyValues[index]["QuoteLineId"].ToInt();
                    int nextItemId = item.OwnerTableView.DataKeyValues[index - 1]["QuoteLineId"].ToInt();
        ...
        protected void btnMoveDownSub_Click(object sender, System.Web.UI.ImageClickEventArgs e)
        {
            try
            {
                ImageButton downbtn = (ImageButton)sender;
                GridDataItem item = (GridDataItem)downbtn.NamingContainer;
                int index = item.ItemIndex;
                if (index + 1 < item.OwnerTableView.Items.Count)
                {
                    int movedItemId = item.OwnerTableView.DataKeyValues[index]["QuoteLineId"].ToInt();
                    int nextItemId = item.OwnerTableView.DataKeyValues[index + 1]["QuoteLineId"].ToInt();
        ...
        public void ExpandGroup(GridTableView tableView)
        {
        ...
        public void CloseExpandGroup(GridTableView tableView)
        {
            GridItem[] nestedViewItems = tableView.GetItems(GridItemType.NestedView);
            foreach (GridNestedViewItem nestedViewItem in nestedViewItems)
            {
        ...
        {
            CloseExpandGroup(rgQuote.MasterTableView);
        }

        protected void imgbtnOpen_Click(object sender, ImageClickEventArgs e)
        {
            ExpandGroup(rgQuote.MasterTableView);
        }
