...
{
    public partial class ModificationOrderList : System.Web.UI.Page
    {
        readonly ModificationOrderBLL _modif = new ModificationOrderBLL();
        ...
        protected void GetLookupTables()
        {
            try
            {
                ...
                ddlStatus.DataSource = lookup.GetModificationOrderHeaderStatus();
                ddlStatus.DataBind();
        ...
        {
            rgModificationOrderList.Rebind();
        }

        private DataTable GetModificationOrderList()
        {
            try
            {
                return _modif.GetModificationOrderList(DateStart, DateEnd, JobOrderNumber, StatusId);
            }
            catch (Exception ex)
            {
                AppNotification.MessageBoxException(ex);
        ...
        protected void rgModificationOrderList_Init(object sender, EventArgs e)
        {
            var grid = (RadGrid)sender;
            grid.PageSize = CommonMember.GridPageSize;
        }

        protected void rgModificationOrderList_NeedDataSource(object sender, GridNeedDataSourceEventArgs e)
        {

            rgModificationOrderList.DataSource =  GetModificationOrderList();
        }

        protected void rgModificationOrderList_ItemDataBound(object sender, GridItemEventArgs e)
        {
        ...
        protected void ExportToExcelFile()
        {
            try
            {
                ...
                DataTable dt = GetModificationOrderList();

                dt.Columns.Remove("ModificationOrderId");
                ...
                excel.ExportExcel(dt, ExcelType.Xls, "Modification Order List", "Modification_Order ");
            }
            catch (Exception ex)
            {
                AppNotification.MessageBoxException(ex);
