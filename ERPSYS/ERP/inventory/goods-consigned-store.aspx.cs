...
{
    public partial class GoodsConsignedStore : System.Web.UI.Page
    {
        ...
        private void ShowMessages()
        {
            try
            {
                ...
                {
                    case "1":
                        AppNotification.MessageBoxFailed(GeneralResources.GetStringFromResources("goods_consigned_item_id_not_exist"));
                        break;
                }
            }
            catch (Exception ex)
        ...
        private DataTable GetGoodsConsignedStoreList()
        {
            try
            {
        ...
        protected void rgInvItemList_NeedDataSource(object sender, GridNeedDataSourceEventArgs e)
        {
            try
            {
                rgInvItemList.DataSource = GetGoodsConsignedStoreList();
            }
            catch (Exception ex)
            {
                AppNotification.MessageBoxException(ex);
        ...
        protected void ExportToExcelFile()
        {
            try
            {
                ...
                DataTable dt = GetGoodsConsignedStoreList();
                ...
                excel.ExportExcel(dt, ExcelType.Xls, "Consigned Goods Store", "Consigned_Goods");
            }
            catch (Exception ex)
            {
                AppNotification.MessageBoxException(ex);
